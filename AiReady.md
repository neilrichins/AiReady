...
> Copy this file to the root of the project being assessed. Replace every `{{PLACEHOLDER}}`, attach verifiable evidence, and record unresolved limitations. This assessment is a point-in-time engineering record, not a certification.
...
| Control | Score | Evidence / limitation |
| --- | ---: | --- |
| A root `AGENTS.md` defines objectives, invariants, workflow, commands, and prohibited actions | `{{0_1_2}}` | `{{EVIDENCE}}` |
| Large or specialised directories have scoped README files | `{{0_1_2}}` | `{{EVIDENCE}}` |
